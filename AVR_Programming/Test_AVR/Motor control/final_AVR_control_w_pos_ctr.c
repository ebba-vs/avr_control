...
volatile int16_t desired_pos = 150;
volatile int16_t dist_error = 0;
volatile int16_t pos_u = 0;
...
// Initialize Analog-to-Digital Converter (ADC) for PC0
int init_ADC(void)
{
	DDRC &= ~(1 << DDC0); // enable PC0 as a input

	ADMUX = (1 << REFS0) | 0; // sets VRef to VCC and enables ADC on PC0
...
uint16_t get_dist(int16_t adc) {
	if(adc > 243) return 125;
	if(adc < 106) return 225;
	uint16_t k_inv = 277;   // 1.081 * 256 = 277 Q8.8 // 
	//if (diff < 0) diff = 0;                        // clamp (outside range)

	uint32_t mm_q = k_inv * (355 - adc);        // Q8.8
	uint16_t mm = mm_q >> n; // round + convert

	return mm;  // distance in mm
...
//
			...
			//}
		//}
		//if (!pos && pi_tick && pi_enabled && rpm_ref > 0 && rpm_ref < 120)
		//{
			//pi_tick = 0;
			//rpm_acc = calc_rpm();
...
//
			//uint8_t check = (uint8_t)updatePI(rpm_acc, new_target); // uses rpm_ref and rpm_meas
			//OCR0A = check;
			//if (j >= 15)
			//{
				 //USART_Transmit(rpm_acc);
				 //USART_Transmit(check);
				//USART_Transmit(new_target);
				//j = 0;
			//}
			//j++;
		//}
...
int main(void) {
	...
	while(1){
		...
		adc_raw = read_ADC();
		curr_pos = get_dist(adc_raw);

		dist_error = desired_pos - curr_pos;
		//int16_t pos_u = 0;
		if((dist_error <= 5) && (dist_error >= -5)) {
			pos_u = 0;
		}
		else {
			pos_u = dist_error * 3;
