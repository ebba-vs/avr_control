...
void timer2_init() {
	...
	TCCR2B = (1 << CS22) | (1 << CS20); //prescaler = 128 --> f_ovf = 15,3Hz. precaler_256 wold give fovf = 30
...
void updatePI(uint8_t rpm_acc, uint8_t rpm_ref) {
	int16_t e = (int16_t)rpm_ref - (int16_t)rpm_acc;  // signed
	int16_t Kp_q = (int16_t)(Kp * Q_SCALE);
	int16_t Ki_q = (int16_t)(Ki * Q_SCALE);
	...
	int32_t I_min_q = ((int32_t)U_MIN) << n;	//
	int32_t I_max_q = ((int32_t)U_MAX) << n;
	if (I_q > I_max_q) {
		I_q = I_max_q;
	}
	if (I_q < I_min_q)  {
...
int main(void) {
	...
	while (1) {
		if (new_cmd) {
			uint8_t rpm_ref = received_data; // copy to local
			uint8_t rpm_acc = calc_rpm();
			new_cmd = 0;
			...
			if (rpm_ref == 0) {
				...
				if (pi_tick) {
					...
					set_LED(3,0);
					pi_tick = 0;
					updatePI(rpm_acc, rpm_ref);   // uses rpm_ref and rpm_meas
					USART_Transmit(0);
					} else {
					// optional: hold PWM at 0 while not enabled
					set_LED(1,0);
				}
			}
