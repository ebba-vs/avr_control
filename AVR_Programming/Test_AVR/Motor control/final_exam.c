...
int16_t Kp_q = (int16_t)(0.8 * Q_ctr_SCALE);	  // fixed point proportional gain
int16_t Ki_q = (int16_t)(0.02 * Q_ctr_SCALE);	  //fixed point integral gain
int16_t Ts_q = (int16_t)(0.065536 * Q_ctr_SCALE); // sample time Ts = 0.065536

int32_t I_q = 0; // integrator in Q8.8 (PWM units)

volatile uint16_t dt_ticks = 0;		// ticks between encoder edges
volatile uint16_t motorspeed_q = 0; // RPM 
...
// 60 * F_CPU * 2^8 / (prescaler * edges)
static const uint32_t K_rpm = (uint32_t)((60ULL * F_CPU * (uint32_t)Q_SCALE) /
										 (TIMER1_PRESCALER * EDGES_PER_REV)); // real value: 78125 for ps=8, F_CPU=1Mhz
...
// Read the ADC value from a specified channel (PC0 = channel 0)
...
uint8_t calc_rpm()
{
	...
	if (dt_tick != 0)
	{
		...
		//{
			//q = MAX_RPM_Q;
		//}
		rpm_meas = (uint8_t)(q >> n); // right shift 8 -> real number
	}
	return rpm_meas;
}
int8_t updatePI(uint8_t speed_acc, uint8_t speed_ref)
{
	...
	int32_t P_q = (int32_t)Kp_q * (int32_t)e; 
